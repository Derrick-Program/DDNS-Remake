...
install_client() {
    ...
    prompt CHECK_INTERVAL "IP check interval (seconds)" "60"
...
EOF
    chown -R "${SERVICE_USER}" "${cfg_dir}"
    chmod 600 "${cfg_file}"
    success "Initial config written to ${cfg_file}"

    # Run auth login as the service user so the token is saved to the right config path
    # ddns-client auth login handles: username/password → JWT → device registration → domain selection
    echo ""
    bold "Device Registration"
