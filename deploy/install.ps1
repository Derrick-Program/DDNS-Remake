...
function Install-Client {
    ...
    $deviceToken    = Prompt-Value "Device API token (from 'ddns-server auth exchange')" ""
    $checkInterval  = Prompt-Value "IP check interval (seconds)" "300"
...
"@ | Set-Content -Encoding UTF8 $cfgFile
    ...
    Write-Bold "`nNext steps:"
    Write-Host "  1. Review config : $cfgFile"
    Write-Host "  2. Check status  : Get-Service DuacodieClient"
    Write-Host "  3. View logs     : Get-EventLog -LogName Application -Source DuacodieClient"
}
